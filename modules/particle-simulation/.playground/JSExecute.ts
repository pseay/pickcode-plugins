...
function sanitizeMessage(value: any) {
    // Keep a stack to detect cycles
    const seen = new WeakSet();

    // Recursively walk the object and replace problematic values
    function replacer(key: string, val: any) {
        if (typeof val === "object" && val !== null) {
            // If we've seen this object before, break the cycle
            if (seen.has(val)) {
                return "[Circular]";
            }
            seen.add(val);
    ...
    // Convert to string, then parse back to a regular object
    // so it can be passed safely to postMessage
...
const overrideGlobalFns = (onTermination: () => void) => {
    ...
    const maybeTerminate = () => {
        if (timeouts.size === 0 && intervals.size === 0) {
            // Use the promise to delay sending the message until this
    ...
    // TODO: override Promise too! To track if any code got deferred
    // and is now running in a separate "thread"
...
const handleMessage = async (message: any) => {
    switch (message.type) {
        case "startJS": {
            ...
            // TODO: send back a console message if it can't
            // be parsed, etc
            let userCode;
            try {
                ...
                if (e instanceof Error) {
                    // It's apparently impossible to get the line number or
                    // other context from JS itself. We'd have to use a
                    // parsing library
                    postMessage({
                        type: "module",
                        contents: {
