...
const createExports = (sendMessage: (message: SimulationMessage) => void) => {
    return Promise.resolve({
        runSimulation: (getNextPosition: (currentPosition: { x: number, y: number }, lastVelocity: { x: number, y: number }, timestep: number) => { x: number, y: number }) => {
            ...
            // Initial conditions
            let currentPositionPredicted = { x: 0, y: 1 }; // Start at 1m height
            let currentVelocityPredicted = { x: 30, y: 10 }; // Initial velocity (m/s)
            ...
            const getNextPositionActual = (currentPos: { x: number, y: number }, lastVel: { x: number, y: number }, dt: number) => {
                ...
                return { position: { x: nextPosX, y: nextPosY }, velocity: { x: currentVelX, y: currentVelY } };
            };

            const interval = setInterval(() => {
                if (currentPositionActual.y < 0 && currentPositionPredicted.y < 0) {
                    isComplete = true;
                ...
                if (!isComplete) {
                    ...
                    const nextPredicted = getNextPosition(currentPositionPredicted, currentVelocityPredicted, TIMESTEP);
                    ...
                    currentVelocityPredicted.x = (nextPredicted.x - currentPositionPredicted.x) / TIMESTEP;
                    currentVelocityPredicted.y = (nextPredicted.y - currentPositionPredicted.y) / TIMESTEP;
                    currentPositionPredicted = nextPredicted;

                    // Actual path
                    const actualResult = getNextPositionActual(currentPositionActual, currentVelocityActual, TIMESTEP);
